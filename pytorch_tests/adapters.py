...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    torch.nn.utils.clip_grad_norm_(parameters, float(max_l2_norm))
...
def get_tokenizer(
    ...
    """Given a vocabulary, merges, and optional special tokens, return a tokenizer instance.

    Args:
        vocab (dict[int, bytes]): Token ID to token-bytes mapping.
        merges (list[tuple[bytes, bytes]]): Ordered BPE merges.
        special_tokens (list[str] | None): Optional list of special token strings.

    Returns:
        A tokenizer that uses the provided vocab and merges.
    """
    return tokenizer.init_tokenizer(vocab, merges, special_tokens)
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    """Adapter wrapper for training a byte-level BPE tokenizer."""
    return train_bpe.train_bpe(input_path=input_path, vocab_size=vocab_size, special_tokens=special_tokens, **kwargs)
